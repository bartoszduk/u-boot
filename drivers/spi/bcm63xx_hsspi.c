		...
		const void *dout, void *din, unsigned long flags)
{
	...
	if (flags & SPI_XFER_BEGIN) {
		...
		clrsetbits_be32(priv->regs + SPI_PFL_SIG_REG(plat->cs),
				clr, set);
	...
	if (flags & SPI_XFER_END)
		clrsetbits_be32(priv->regs + SPI_CTL_REG,
				SPI_CTL_CS_POL_MASK,
				priv->cs_pols);
...
static int bcm63xx_hsspi_probe(struct udevice *dev)
{
	...
	/* enable clk gate */
	val = readl_be(priv->regs + SPI_CTL_REG);
	val |= SPI_CTL_CLK_GATE_MASK;
	writel_be(val, priv->regs + SPI_CTL_REG);
