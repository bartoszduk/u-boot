...
static int bcm6348_eth_start(struct udevice *dev)
{
	...
	if (ret) {
		error("could not initialize phy\n");
		return ret;
	}
...
static int bcm6348_mii_soft_reset(struct udevice *dev)
{
	struct bcm6348_eth_priv *priv = dev_get_priv(dev);
	struct mii_dev *bus;
	uint32_t timeout = ETH_TIMEOUT_US;
	...
	printf("%s: mii soft reset phy %u\n", __func__, priv->phy_id);

	/* request mii restart */
	if (bcm6348_mdio_write(bus, priv->phy_id, MDIO_DEVAD_NONE, MII_BMCR,
			       BMCR_RESET)) {
		error("error requesting mii reset\n");
		return -EINVAL;
	}
	wmb();

	/* wait for mii restart */
	udelay(20);
	do {
		uint32_t val = bcm6348_mdio_read(bus, priv->phy_id,
						 MDIO_DEVAD_NONE, MII_BMCR);
		rmb();

		if (val < 0) {
			error("error checking mii reset\n");
			return -EINVAL;
		}
	...
	printf("%s: mii reset in %u us (timeout = %u)\n", __func__, ETH_TIMEOUT_US - timeout, timeout);

	mdelay(1000);

	if (!timeout)
		error("mii reset timeout\n");
...
static int bcm6348_phy_init(struct udevice *dev)
{
	...
	/* request mii restart */
#if defined(COONFIG_SOC_BMIPS_BCM6348)
	setbits_be32((void __iomem *) 0xfffe0418, 0x7007);
#elif defined(CONFIG_SOC_BMIPS_BCM6358)
	setbits_be32((void __iomem *) 0xfffe0098, 0x80000000);
#endif
	bcm6348_mii_soft_reset(dev);

	/* phy connect */
	priv->phy_dev = phy_connect(bus, priv->phy_id, dev, pdata->phy_interface);
	if (!priv->phy_dev) {
...
#if 0
	{
		...
		if (phy_write(priv->phy_dev, priv->phy_id, MII_BMCR, BMCR_RESET) < 0) {
			error("PHY reset timed out\n");
		}

		udelay(20);

		reg = phy_read(priv->phy_dev, priv->phy_id, MII_BMCR);
